...
const YTX = {
  ...
  // 병목은 토큰이 아니라 "지연"이므로, 청크를 키워 요청 수를 줄이는 편이 유리하다.
  // FIRST: 첫 청크는 작게 잘라 영상 시작 부분이 가장 먼저 번역·표시되게 한다.
  // PARALLEL: 동시 처리 청크 수 — "청크 출력 토큰 × 병렬 수"가 계정의 분당
  // 처리율(TPM)을 넘으면 생성 도중 끊기므로(레이트리밋 재시도 반복의 원인) 주의.
  // Udemy(transcriptByU) 참고: 그쪽 claude-code 경로는 60세그 청크를 서버 동시 3개로
  //   굴린다(≈180세그 동시). YouTube는 청크를 키우되(80→120) 동시 토큰 부하를 비슷하게
  //   두려고 병렬은 2로 유지 → 동시 ≈ 120×2 = 240세그(출력 ~20K 토큰, Udemy와 같은 자릿수).
  //   더 공격적으로: PARALLEL을 3으로(서버 MAX_CONCURRENT=3까지). 레이트리밋 재시도가
  //   잦아지면 SEGMENTS나 PARALLEL을 다시 낮춘다.
  ...
  LOCALHOST_TIMEOUT_MS: 180000, // 청크 확대(120세그)로 생성시간 증가 대응 (Gemini 경로와 동일한 180s)
